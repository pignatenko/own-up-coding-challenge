import { Grid, Button, InputAdornment, TextField, Input, MenuItem, InputLabel, Select } from '@material-ui/core';
...
function displayEnum(_enum, displayEnum) {
  return Object.values(_enum).map(id => (<MenuItem key={id} value={id}>{displayEnum[id]}</MenuItem>))
}
...
export function RateQuoteControls() {
  ...
  const handleLoanSize = debounce(values => dispatch(Actions.setLoanSize(values.floatValue)), 100);

  const { propertyType, occupancy, loanSize, creditScore } = useSelector(s => s.rateQuotes);
  const isCreditScoreValid = () => 300 <= creditScore && creditScore <= 800;
  const creditScoreHelperText = () => !isCreditScoreValid() ? 'must be between 300 and 800' : null;
  ...
  return (
    <div className="RateQuoteControls">
      <Grid container direction="row" justify="space-between" alignItems="flex-start" spacing={1}>
        <Grid item container xs={6} direction="column" justify="space-evenly" alignItems="flex-start" spacing={5}>
          <Grid item>
          ...
          <Grid item>
            <InputLabel htmlFor="rate-quotes-credit-score">Credit Score</InputLabel>
            <NumberFormat
              format="###"
        ...
        <Grid item container xs={6} direction="column" justify="space-evenly" alignItems="flex-end" spacing={5}>
          <Grid item>
            <InputLabel htmlFor="rate-quotes-property-type">Property Type</InputLabel>
            <Select id="rate-quotes-property-type"
              value={propertyType}
          ...
          <Grid item>
            ...
            <Select id="rate-quotes-occupancy"
              value={occupancy}
              name="setOccupancy"
              onChange={handleChange}
            >
          ...
          <Grid item>
            <Button id="rate-quotes-quote-rates" disabled={!isFormValid()} variant="contained" color="primary" onClick={() => dispatch(Actions.getRateQuotes(loanSize, creditScore, propertyType, occupancy))}>
              Quote Rates
            </Button>
          </Grid>
        </Grid>
